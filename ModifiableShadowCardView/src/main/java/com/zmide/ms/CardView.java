...
public class CardView extends FrameLayout {
	...
	{
		IMPL.setShadowColor(color,0x03000000);
	}

	public void setShadowColor( int startColor, int endColor)
	{
		IMPL.setShadowColor( startColor, endColor);
	}

	public int getShadowStartColor()
	{
		return IMPL.getShadowStartColor();
	}

	public int getShadowEndColor()
	{
		return IMPL.getShadowEndColor();
	}
    ...
    private void initialize(Context context, AttributeSet attrs, int defStyleAttr) {
		...
		// find the shadow color
		if (a.hasValue(R.styleable.CardView_shadowColor)) {
             mStartColor = a.getColor(R.styleable.CardView_shadowColor,0x03000000);
        } else {
            // There isn't one set, so we'll compute one based on the default shadow color
			IMPL.setShadowColor(0x03000000,0x03000000);
			}
